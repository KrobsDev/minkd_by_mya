...
export function getCustomerReminderEmail(data: BookingEmailData): string {
  const customerName = escapeHtml(data.customerName);
  const serviceName = escapeHtml(data.serviceName);
  const appointmentDate = escapeHtml(data.appointmentDate);
  const appointmentTime = escapeHtml(data.appointmentTime);
  const bookingReference = escapeHtml(data.bookingReference);
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Appointment Reminder - Mink'd by Mya</title>
</head>
<body style="margin: 0; padding: 0; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background-color: #FFF8FC;">
  <table role="presentation" style="width: 100%; border-collapse: collapse;">
    <tr>
      <td align="center" style="padding: 40px 0;">
        <table role="presentation" style="width: 600px; border-collapse: collapse; background-color: white; border-radius: 16px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
          ...
          <tr>
            <td style="padding: 40px;">
              <h2 style="margin: 0 0 20px; color: #1F2937; font-size: 24px;">Appointment Reminder</h2>
              <p style="margin: 0 0 20px; color: #4B5563; font-size: 16px; line-height: 1.6;">
                Hi ${customerName},
              </p>
              <p style="margin: 0 0 30px; color: #4B5563; font-size: 16px; line-height: 1.6;">
                This is a reminder that your Mink'd by Mya appointment is scheduled for tomorrow.
              </p>
                ...
                <tr>
                  <td style="padding: 24px;">
                    ...
                    <table role="presentation" style="width: 100%; border-collapse: collapse;">
                      <tr>
                        <td style="padding: 8px 0; color: #6B7280; font-size: 14px;">Service:</td>
                        <td style="padding: 8px 0; color: #1F2937; font-size: 14px; font-weight: 600; text-align: right;">${serviceName}</td>
                      </tr>
                      <tr>
                        <td style="padding: 8px 0; color: #6B7280; font-size: 14px;">Date:</td>
                        <td style="padding: 8px 0; color: #1F2937; font-size: 14px; font-weight: 600; text-align: right;">${appointmentDate}</td>
                      </tr>
                      <tr>
                        <td style="padding: 8px 0; color: #6B7280; font-size: 14px;">Time:</td>
                        <td style="padding: 8px 0; color: #1F2937; font-size: 14px; font-weight: 600; text-align: right;">${appointmentTime}</td>
                      </tr>
                      <tr>
                        <td style="padding: 8px 0; color: #6B7280; font-size: 14px;">Reference:</td>
                        <td style="padding: 8px 0; color: #BE185D; font-size: 14px; font-weight: 600; text-align: right;">${bookingReference}</td>
                      </tr>
                    </table>
                  </td>
                </tr>
              ...
              <div style="background-color: #FEF3C7; border-radius: 8px; padding: 16px; margin-bottom: 30px;">
                <p style="margin: 0; color: #92400E; font-size: 14px; line-height: 1.5;">
                  <strong>Please note:</strong> Kindly arrive 5-10 minutes before your appointment. If you need to reschedule or cancel, please contact us as soon as possible.
                </p>
              </div>
